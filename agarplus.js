...
function MyApp() {
    ...
    this.isShowPacketIO = this.isEnableShareFb = this.isEnableSound = this.isHide
    Name = this.testing = false;
    this.build = function() {
    ...
    this.init = function() {
       $("body").html("");
        ...
        document.title = "Xgario.ml";
        $("body").append("<div id = 'overlays2'></div>");
        $("#overlays2").append("<div id = 'div_lb'><div class='header'>Leaderboard</div></div>");
    ...
    this.sendMessage = function(msg) {
        if (msg = msg.trim()) {
            if (!(2E3 > Date.now() - t && 50 > msg.length)) {
                ...
                this.lastMsg = msg;
                t = Date.now();
                console.log("Message: " + this.sendMessage);
            }
        }
    };
...
function Connection() {
    ...
    self.connect = function() {
        socket2 = io("ws://134.255.227.184:4040", {
            transports: ["websocket"]
        });
        socket2.on("updateCoords", function(walkers) {
            minimap.updateNode(walkers);
        });
        socket2.on("receiveMessage", function(data) {
            chatRoom.receiveMessage(data.sender, data.msg);
        });

        socket2.on("ping", function(data) {

            var type = data.type;
            var ping = data.ping;

            if (type == 0 && ping == 1) { //JOIN PING
    ...
    self.joinChat = function(val) {
        self.joinChatRoom(val);
    }

    window.joinChat = self.joinChat;
    window.createPrivateChat = self.createPrivateChat;

    self.joinChatRoom = function(value) {
    ...
    self.joinRoom = function(value) {
        //self.joinChatRoom(myApp.getCurrentIP()); Iam back
    };
...
var toastQueue = [];
var defaultSkin = "http://i.imgur.com/B3fgpDn.jpg";
var gm;
    ...
    */
    ...
    function fn(view, offset) {
        ...
        for (; id = view.getUint32(offset, true), offset += 4, 0 != id;) {
            ...
            if (skin) {
                item.skin = skin;
                //console.log("I HAVE A SKIN => " + skin);
            }
            if (input) {
                item.t(input);
            }
    ...
    function writeUTFBytes() {
        if (handler() && (matchEnd && null != b)) {
            ...
            var buf = encode(1 + 2 * b.length + (test.length * 2));
            buf.setUint8(0, 0);
            var bi = 0;
            var a = 0;
    ...
    if (self.moveTo = function(lab, dragging) {
            if (lab) {
                if (dragging) {
                    myApp.isStopMovement = true;
                }
                    ...
                    s: function(ctx) {
                        if (this.H()) {
                            ...
                            if (20 > this.size) {
                                ...
                                ++this.S;
                                var y_position = 0 < this.id && (!this.f && (!this.j && 0.05 > scale));
                                ...
                                if (v = this.name + this.color, v = v in playerDetailsByIdentifier ? playerDetailsByIdentifier[v] : void 0, i) {
                                    ...
                                    if (isHideSelfName) {
                                        ...
                                        if (this.skin) {
                                            ...
                                            //	console.log("My cell have an aswome skin => " + this.skin);
                                            x = this.skin;
                                ...
                                if (skinName != null) {
                                    ...
                                    var new_name = this.skin.split("%")[1];


                                    if (myApp.isEnableOtherSkinSupport && skinName != '' && new_name != undefined) {
                                        if (!results.hasOwnProperty(new_name)) {
                                            results[new_name] = new Image;

                                            var https = false;
                                            var http = false;

                                            if (new_name.split("http://")[1] != undefined) {

                                                http = true;

                                            } else if (new_name.split("https://")[1] != undefined) {
                                            ...
                                            if (http || https) {
                                                ...
                                                results[new_name].src = new_name;

                                            } else {

                                                results[new_name].src = "https://i.imgur.com/" + new_name;

                                            }

                                        }
                                        if (0 != results[new_name].width && results[new_name].complete) {
                                            c = results[new_name];
                                        } else {
                                            c = null;
                                        }
                                ...
                                y_position || f && (this.f && ctx.stroke())

                                ctx.fill()

                                /*if (ctx.closePath(), i = this.name.toLowerCase(), c = this.img ? "http://upload.happyfor.me/getimg.php?id=" + this.img + "&_t=" + Math.random() : "skins/" + i + ".png", x || (this.j || (!error && !myApp.isEnableOtherSkinSupport || ":teams" == index)) ? n = null : (n = this.V, null == n ? n = null : ":" == n[0] ? (images.hasOwnProperty(n) || (images[n] = new Image, images[n].src = n.slice(1)), n = 0 != images[n].width && images[n].complete ? images[n] : null) : n = null, n || (-1 !=
...
}(window, window.jQuery), myApp.afterGameLogicLoaded(), $(document).keydown(function(e) {
...
}), $.cachedScript("http://extension.agarplus.io/jquery.toast.min.js").done(function(dataAndEvents, deepDataAndEvents) {
    var restoreScript;
